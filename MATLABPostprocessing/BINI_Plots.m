...
% Plot flowrates
figure('defaultAxesFontSize', 9, 'defaultLineLineWidth', 1, ...
       'Units', 'inches', 'Position', [1, 1, 7, 2]);
...
ylabel('$Q(t)_{Aorta} \ (mL/s)$', 'Interpreter', 'latex');
ax = gca;
...
xlim([0 1]);
legend('Inlet','Outlet', 'Interpreter','latex', 'location', 'northeast', 'Box', 'off')
...
plot(Coronary_Qout2(:,1), Coronary_Qout2(:,2) * 1e6, '--r', 'LineWidth', 1.5);
ylabel('$Q(t)_{LCA} \ (mL/s)$', 'Interpreter', 'latex');
ax = gca;
ax.YColor = 'k'; % Set left axis color to red
ax.TickLabelInterpreter = 'latex'; % Use LaTeX for tick labels
ylim([0.15 0.7]);
...
nexttile;
hold on;
h(1)=plot(R_tSol, R_PSol / 133.33, '-k', 'LineWidth', 1.5); % Pressure plot
h(2)=plot(WK2_tSol, WK2_PSol / 133.33, '-k', 'LineWidth', 1.5); % Pressure plot
h(3)=plot(WK3_tSol, WK3_PSol / 133.33, '-k', 'LineWidth', 1.5); % Pressure plot
h(4)=plot(WK4s_tSol, WK4s_PSol / 133.33, '-k', 'LineWidth', 1.5); % Pressure plot
h(5)=plot(WK4p_tSol, WK4p_PSol(:,1) / 133.33, '-k', 'LineWidth', 1.5); % Pressure plot
h(6)=plot(R_Pout(:,1), R_Pout(:,2) /133.33, '--r', 'LineWidth', 1.5);
h(7)=plot(WK2_Pout2(:,1), WK2_Pout2(:,2) /133.33, '--b', 'LineWidth', 1.5);
h(8)=plot(WK3_Pout2(:,1), WK3_Pout2(:,2) /133.33, '--c', 'LineWidth', 1.5);
h(9)=plot(WK4s_Pout2(:,1), WK4s_Pout2(:,2) /133.33, '--m', 'LineWidth', 1.5);
h(10)=plot(WK4p_Pout2(:,1), WK4p_Pout2(:,2) /133.33, '--g', 'LineWidth', 1.5);
ylabel('$P(t)_{Aortic \ Outlet} \ (mmHg)$', 'Interpreter', 'latex');
ax = gca;
ax.YColor = 'k'; % Set left axis color to red
ax.TickLabelInterpreter = 'latex'; % Use LaTeX for tick labels
ylim([60 140]);
xlabel('$t \ (s)$', 'Interpreter', 'latex');
xlim([7 8]);
legend(h(5:10),'MATLAB','R','RC','RCR','RCRL-s','RCRL-p', 'Interpreter','latex', 'location', 'northeast', 'Box', 'off')
set(gca, 'XMinorTick', 'on', 'YMinorTick', 'on');
grid on; grid minor; box on;
% Adjust figure for better visibility
set(gca, 'FontSize', 9);
...
ylabel('$P(t)_{LCA \ Outlet} \ (mmHg)$', 'Interpreter', 'latex');
ax = gca;
ax.YColor = 'k'; % Set left axis color to red
ax.TickLabelInterpreter = 'latex'; % Use LaTeX for tick labels
%ylim([60 140]);
xlabel('$t \ (s)$', 'Interpreter', 'latex');
xlim([7 8]);
legend('MATLAB','OpenFOAM', 'Interpreter','latex', 'location', 'northeast', 'Box', 'off')
set(gca, 'XMinorTick', 'on', 'YMinorTick', 'on');
grid on; grid minor; box on;
% Adjust figure for better visibility
set(gca, 'FontSize', 9);
...
% plot(PimTime, PimScaling*Pim /133.33, '-m', 'LineWidth', 1.5);
% xlabel('$t \ (s)$', 'Interpreter', 'latex');
% ylabel('$P(t) \ (mmHg)$', 'Interpreter', 'latex');
% %title('Pressure Solution', 'Interpreter', 'latex');
% legend('MATLAB', '$O(\Delta t)$ Coronary LPN BC', '$O(\Delta t^2)$ Coronary LPN BC', 'Interpreter', 'latex', 'location', 'southeast', 'Box', 'off');
% ax = gca;
% ax.YColor = 'k'; % Set left axis color to blue
% ax.TickLabelInterpreter = 'latex'; % Use LaTeX for tick labels
% set(gca, 'XMinorTick', 'on', 'YMinorTick', 'on');
